...
def analyze_image_intent(img_path, cfg):
    try:
        result = vision_request(INTENT_PROMPT, images=[str(img_path)])
        if result:
...
def run(photos_dir=None, enable_viral_research=True):
    cfg = load_config()
    ...
    for image in image_files:
        ...
        intent = analyze_image_intent(image, cfg)
